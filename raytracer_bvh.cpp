...
void subdivide(BVHNode* node, int left) {
  ...
  //  cout << "BVH Index: " << bvhIndex << endl;
  //  cout << "BVH Size: " << bvhNodes.size() << endl;
...
// bool rayAABBIntersect2( Point rayOrigin, Dir rayDir, Vec3 min, Vec3 max ) {
//     Vec3 t0 = (min - Vec3(rayOrigin)) * Vec3(rayDir);
//     Vec3 t1 = (max - Vec3(rayOrigin)) * Vec3(rayDir);

//     Vec3 vmin = Vec3(fmin(t0.x, t1.x), fmin(t0.y, t1.y), fmin(t0.z, t1.z));
//     Vec3 vmax = Vec3(fmax(t0.x, t1.x), fmax(t0.y, t1.y), fmax(t0.z, t1.z));
...
Intersect raySphereIntersect(Point rayOrigin, Dir rayDir, Point sphereCenter, float sphereRadius){
    float r2 = pow(sphereRadius, 2);
    bool inside = (sphereCenter - rayOrigin).magnitudeSqr() < r2;
    float t = 0;
    if(!inside) {
        float tc = dot((sphereCenter - rayOrigin), rayDir) / rayDir.magnitude();
        if (tc < 0) return Intersect();
        float d2 = (rayOrigin + tc*rayDir - sphereCenter).magnitudeSqr();
        if(r2 < d2) return Intersect();
    ...
    return Intersect(t * rayDir + rayOrigin, t, Sphere(sphereCenter, sphereRadius));
}

bool raySphereShadowIntersect(Point rayOrigin, Dir rayDir, Point sphereCenter, float sphereRadius, Point hit, Point lightPos){
    float r2 = pow(sphereRadius, 2);
    bool inside = (sphereCenter - rayOrigin).magnitudeSqr() < r2;
    float t = 0;
    if(!inside) {
...
Intersect rayBVHIntersect( int nodeIdx, Point rayOrigin, Dir rayDir, Dir rayInvDir ) {
   // cout << "302" << endl;
    BVHNode* node = bvhNodes[nodeIdx];
   // cout << "304" << endl;

   // cout << "Node index: " << nodeIdx << endl;


    if (!rayAABBIntersect( rayOrigin, rayInvDir, node->aabb.min, node->aabb.max )) return Intersect();
    ...
    if (node->count > 0) {
       // cout << "Leaf node count: " << node->count << endl;
        ...
        for (int i = 0; i < node->count; i++ ) {
          //  iters++;
           // cout << "i: " << i << endl;
           // cout << "index: " << node->indices[i] << endl;

            Intersect intersect = raySphereIntersect( rayOrigin, rayDir, spheres[node->indices[i]].center, spheres[node->indices[i]].radius );
           // cout << "318" << endl;
...
// Intersect rayBVHIntersect( BVHNode* node, Point rayOrigin, Dir rayDir, Dir rayInvDir ) {
...
bool shadowRayBVHIntersect( int nodeIdx, Point rayOrigin, Dir rayDir, Dir rayInvDir, Point hit, Point lightPos ) {
    BVHNode* node = bvhNodes[nodeIdx];
    ...
    if (node->count > 0) {
        for (int i = 0; i < node->count; i++ ) {
            ...
            if (raySphereShadowIntersect(rayOrigin, rayDir, spheres[node->indices[i]].center, spheres[node->indices[i]].radius, hit, lightPos)) 
                return true;
        }
        return false;
    }
...
Intersect rayTriangleIntersect(Ray ray, Triangle triangle) {
    ...
    if(t > HIT_THRESH) {
        ...
        return Intersect(ray.origin + t*ray.dir, t, triangle, triNorm);
    }
...
Intersect findNearestSphere(Ray ray) {
    ...
    for(int i = 0; i < spheres.size(); i++) {
      //  iters++;
        Intersect intersect = raySphereIntersect(ray.origin, ray.dir, spheres[i].center, spheres[i].radius);
...
Intersect findNearest(Ray ray) {
    //Intersect nearest;
   // cout << "521" << endl;              
    Intersect nearest = rayBVHIntersect(0, ray.origin, ray.dir, ray.invDir);
   // cout << "523" << endl;
...
std::vector<Triangle> initTriangles() {
    ...
    for (int i = 0; i < triMats.size(); i++) {
        ...
        // triangles.at(i).shine = triShines.at(i);
        // triangles.at(i).trans = triTrans.at(i);
    }
    ...
    if(file.is_open()) {
        ...
        while(getline(file, str)) {
            ...
            if(data.compare("png") == 0) {
                ...
                //cout << "Current Trans: " << string(currMat.trans) << endl;
               ...
               // cout << string(sphere) << endl;
                ...
                // triShines.push_back(currShine);
                // triTrans.push_back(currTrans);
...
bool isShadow(Point hit, Dir lightdir, Point lightPos) {
    ...
    Intersect intersect;
    if (useBVH) {
        isShadow = shadowRayBVHIntersect(0, shadowRay.origin, shadowRay.dir, shadowRay.invDir, hit, lightPos);
       // isShadow = shadowRayBVHIntersect(&bvhRoot, shadowRay.origin, shadowRay.dir, shadowRay.invDir, hit, lightPos);
        //float shadowDist = shadowRayBVHIntersect(&bvhRoot, shadowRay, hit, lightPos);
        //if (shadowDist > HIT_THRESH && shadowDist < (lightPos - hit).magnitude()) return true;
        // Intersect shadowIntersect = rayBVHIntersect(&bvhRoot, shadowRay);
        // if (shadowIntersect.dist > HIT_THRESH && shadowIntersect.dist < (lightPos - hit).magnitude()) return true;
    }
    else {
        for(int j = 0; j < spheres.size(); j++) {

            Intersect intersect = raySphereIntersect(shadowRay.origin, shadowRay.dir, spheres[j].center, spheres[j].radius);
...
// Dir refractRay2(Dir rayDir, Dir norm, float nu) {
//     float index = (1.f / nu);
//     Dir n = norm;
//     float a = dot(rayDir, n);
//     // if(a <= 0) {
...
//     //     index = nu;
//     // }
//     float b = index * sqrt(1.f - a);
//     if (b >= 1) return (((2.f * a) * n) - rayDir).normalized();
    
//     float c = sqrt(1.f - index*index*(1.f - a));

//     return (index * (-1.f*rayDir - (a * n)) - (c * n)).normalized();
    ...
    if(nearest.dist < INFINITY) {
        ...
        for (int i = 0; i < lights.size(); i++) {
            ...
            if(lights[i].dir.magnitude() == 0.f) {
                Dir lightDir = lights[i].pos - nearest.point;
                dropoff = 1.f / lightDir.magnitudeSqr();
                lights[i].dir = lightDir.normalized();
            }
            
            if(!isShadow(nearest.point, lights[i].dir, lights[i].pos)) {
                float dotty = fclamp(dot(nearest.norm, lights[i].dir), 0, 1);
               ...
               // newColor = newColor + (Color() - nearest.shine) * nearest.color * light.color * dotty / dropoff;
            }
        }
